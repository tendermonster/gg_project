...
def initial_playermatrix(grid):
    ...
    for player in grid.players:
        ...
        series['c'] = np.append(series['c'], player.keep_c)
        series['p'] = np.append(series['p'], player.keep_p)
    ...
    return pd.DataFrame.from_dict({'id' : series['id'], 'c' : series['c'], 'p' : series['p'],
     'b' : series['b'], 's': series['strategy'], 'money': series['money']})
...
def create_series(): # create series object
        ...
        s_player = np.array([])
        series = {'id' : id_player, 'c' : c_player, 'p' : p_player,
                'b' : b_player, 'money': money_player, 'strategy': s_player}
        return series
